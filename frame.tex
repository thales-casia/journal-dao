...
\begin{figure}[h!]
  \centering
  \includegraphics[width=122px]{assets/govern.jpg}
  \caption{User Roles.}
  \label{fig:userRoles}
\end{figure}


In the Journal DAO framework \ref{fig:userRoles}, after articles are divided into recursive inscriptions, each inscription is assigned a weight. During token minting events, tokens are distributed proportionally based on these weights. As authors, users receive a certain amount of tokens each time they publish an article, and these tokens will generate future finance. For reviewers, previously published articles contribute to increasing the weight of their research field(keywords), which directly influences their decision-making power during peer review, and in turn, affects their token allocation. Even readers can earn rewards; whether a user downloads an article through personal paid access or via institutional access, a small portion of tokens will be distributed according to the weight of the inscriptions, and readers will also participate in decision-making, thus receiving benefits. By participating in the DAO, users gain proposal rights and financial rewards, which incentivizes greater engagement within the framework, ensuring that the system operates autonomously and efficiently.
% 用户用递归铭文划分之后，每一个铭文都是一个权重，在铸币事件的时候，会按照权重分配相应的token。用户作为作者，每次发布文章的时候，都会获得一定数量的token，这些token日后都会创造收益。用户作为审稿人，之前发布的文章会给自己研究的领域增加权重，这个权重会影响自己评审时决策的占比，这个占比又会反过来影响token的获得。用户甚至作为读者都会获得收益，不管用户个人知识付费下载论文，还是用机构下载论文，也都会按照权重分配少量的token，自己也会参与到决策中，也会获得收益。用户在DAO中可以获得权益以及收益，这会激励用户更好的参与到框架中，框架也完成了自治。
...
% 在本框架中，一篇论文的发表就创建了一个小的生态，如图1。创建时候的第一次铸币量非常大，其中包括了作者，审稿人，出版商。这三个群体中，按照责任制定了比例，token会按照这个比例分配。出版商只有一个，不多赘述。一篇文章，一般是由多个作者组成，每个作者都有自己的比例，比如一作和通讯作者会占比多一些。一个作者是由多个铭文组成，根据铭文又可以对占比做出一定的调整。文章在审稿过程中，会有多个审稿人，每个审稿人都有自己的比例。
