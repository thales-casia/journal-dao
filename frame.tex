...
\begin{figure*}[h]
  ...
  \caption{Inscriptions of Articles and User.}
  \label{fig:inscription}
\end{figure*}

In the framework of Journal DAO, an article resides in a block composed of multiple inscriptions. As shown in Figure \ref{fig:inscription}, the article's inscription records the title and abstract, with children for the authors, keywords, figures, citations, and detailed sections of the article. Once the article enters review, the results are documented in the blockchain's inscriptions; after publication, it can be cited by other article. The parent-child relationship of article inscriptions is similar to traditional article management, but in the blockchain, due to the 4MB size limit of a single inscription, recursive inscriptions allow the complete article to be stored on the blockchain, enabling true decentralization in the DAO system. The article's inscriptions also lay the groundwork for subsequent decision-making and incentive mechanisms. Each user profile is composed of research areas (keywords), published articles, reviewed articles and downloaded articles, with the published papers being inscriptions themselves, which in turn influence the user, capturing as many influencing factors as possible.
% 在journal dao的框架中，一篇论文就在一个区块中，这个区块由多个铭文组成。如图2所示，文章的铭文记录了文章的标题以及摘要，此铭文的子铭文为：作者、关键字、图形、引用以及文章的详细章节。文章一旦进入了评审，会把评审的结果也记录在区块链的铭文中；文章发布之后，它还会被别的用户引用。文章铭文的父级与子级关系与传统的论文管理基本一致，但是在区块链中，因为单个铭文的大小限制为4MB，而一篇文章经常超过这个尺寸，使用递归铭文可以真正的把论文存于区块链中，让DAO系统真正的完成去中心化。文章的铭文，也给后面的决策部分以及激励机制创造了条件。每个用户也是由研究领域(关键字)，发表的文章，评审的文章组成的用户画像，尤其是发表的文章，本身就是铭文组成，又会反过来影响用户，尽可能多的记录了更多的影响因素。
...
%本文结合DAO的基本概念，创建了一个适用于学术期刊的DAO框架，使用有效的激励机制，让整个机构实现自治。该框架首先进行铸币操作，将与论文相关的所有参与者（包括审稿人、作者和读者）按照一定的机制分配代币。这些代币可以用来决定提案的权重，本框架则根据代币分配finance资源。区块链技术的应用，使该框架下的学术期刊生态系统中的资源公平和透明分配。持有代币的参与者可以积极参与文章发布过程，例如发布文章、审阅文章和下载文章，从而根据其代币持有量影响财务资源的分配。通过该框架，DAO运营的学术期刊能够营造一个包容和民主的环境，最重要可以给所有参与者相应的利益。
...
% 如自动机1所示，作者发布了文章，审核如果拒稿，作者需要重新投递，一直循环下去。一旦通过了审核，文章就会发布，此时进入了DAO框架，文章可以无限下载以及引用。
...
% change 6 大修改，去掉公式，改用图形表示，此处要有论文小修大修的展示
...
% 在DAO框架中，代币是在关键事件期间铸造的，首先是文章发布，然后按照预定比例分发给参与者。 所有这些流程均通过智能合约执行，确保透明度和可追溯性。 设计良好的代币分配机制构成了整个框架的基础，从而实现了自我维持和自我调节的环境。
...
% change 8 合并到本章开头，讲述token就是finance
...
% 根据公式10所示，第一个下载article的读者，可以分得全部事件的finance。第二个下载的读者，就要从下一个下载事件才能分得finance，第二t个，从第t+1个下载才能分得finance。因为DAO的token总量在持续增加，所以越早的事件，分得的finance越多，越晚分得的越少。因为每一次都收入是逐渐减少的，所以论文一旦下载量比较高时候再下载，可能永远不会跟支出持平。
